...
void helpRender(std::vector<CelestialBody*> c, glm::mat4 m, std::chrono::duration<long long, std::micro> animation_delta_time_us, FPSCameraf camera, bool show_basis)
{
	...
	for (size_t i = 0; i < c.size(); i++)
	{
		...
		glm::mat4 n = c[i]->render(animation_delta_time_us, camera.GetWorldToClipMatrix(), m, show_basis);
		if (!c[i]->get_children().empty()) {
			helpRender(c[i]->get_children(), n, animation_delta_time_us, camera, show_basis);
		}
	}
...
int main()
{
	...
	while (!glfwWindowShouldClose(window)) {
		...
		std::stack <CelestialBodyRef> bodies;
		bodies.push({ &sun, glm::mat4(1.0f) });

		
		
		while (!bodies.empty()) {
			glm::mat4 m = bodies.top().body->render(animation_delta_time_us, camera.GetWorldToClipMatrix(), bodies.top().parent_transform, show_basis);
			if (!bodies.top().body->get_children().empty()) {
				helpRender(bodies.top().body->get_children(), m, animation_delta_time_us, camera, show_basis);
			}
			bodies.pop();
		}
		...
		// nodes.
		//glm::mat4 test = earth.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), glm::translate(glm::mat4(1.0f), glm::vec3(2.0f, 0.0f, 0.0f)), show_basis);
		...
		//moon.render(animation_delta_time_us, camera.GetWorldToClipMatrix(),test , show_basis);
