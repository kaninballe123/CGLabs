...
edaf80::Assignment2::run()
{
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (interpolate) {
			...
			//!        control points.
			auto const control_points = std::array<glm::vec3, 9> {
				glm::vec3(0.0f, 0.0f, 0.0f),
					glm::vec3(1.0f, 1.8f, 1.0f),
					glm::vec3(2.0f, 1.2f, 2.0f),
					glm::vec3(3.0f, 3.0f, 3.0f),
			...
			if (use_linear) {
				...
				circle_rings_transform_ref.SetTranslate(interpolation::evalLERP(control_points[(int)x%size], control_points[(int)(x+1)%size], t - x));
				
				
			}
			else {

				circle_rings_transform_ref.SetTranslate(interpolation::evalCatmullRom(control_points[((int)x + size - 1) % size], control_points[(int)x % size], control_points[(int)(x + 1) % size], control_points[(int)(x + 2) % size], catmull_rom_tension, t-x));
				//! \todo Compute the interpolated position
