                              ...
                              unsigned int const vertical_split_count)
{
	...
	auto const vertices_nb = (horizontal_split_count + 1 ) * (vertical_split_count + 1 );
	...
	float d_width = width/ horizontal_split_count;
	float d_height = height / vertical_split_count;
	auto pos = glm::vec3(temp_width, 0.0f, temp_height);
	int index = 0;
	...
	for (unsigned int i = 0u; i <= horizontal_split_count; ++i) {
		...
		for (unsigned int j = 0u; j <= vertical_split_count; ++j) {
			...
			temp_height += d_height;
			pos = glm::vec3(temp_width, 0.0f, temp_height);
			vertices[index] = pos;
			texcoords[index] = glm::vec3(static_cast<float>(i) / ((static_cast<float>(horizontal_split_count) - 1.0f)), 0.0f,
				static_cast<float>(j) / ((static_cast<float>(vertical_split_count) - 1.0f)));
			++index;
		}
	...
	index = 0;
	auto indices = std::vector<glm::uvec3>(2u * (horizontal_split_count - 1) * (vertical_split_count - 1 ));
	

	for (unsigned int i = 0u; i < horizontal_split_count-1 ; ++i) {

		for (unsigned int j = 0u; j < vertical_split_count-1 ; ++j) {
			indices[index] = glm::uvec3(i * (vertical_split_count + 1) + j,
				(i + 1) * (vertical_split_count + 1) + j,
				(i + 1) * (vertical_split_count + 1) + j + 1);
			++index;

			indices[index] = glm::uvec3(i * (vertical_split_count + 1) + j,
				(i + 1) * (vertical_split_count + 1) + j + 1,
				i * (vertical_split_count + 1) + j + 1);
                                ...
                                unsigned int const latitude_split_count)
{
	...
	auto const vertices_nb = latitude_split_count*longitude_split_count;
	...
	float d_theta = glm::two_pi<float>() /(longitude_split_count - 1.0f);
	float d_phi = glm::pi<float>() / (latitude_split_count - 1.0f);
	...
	for (unsigned int i = 0u; i < longitude_split_count; ++i) {
		...
		for (unsigned int j = 0u; j < latitude_split_count; ++j) {
			...
			// texture coordinates
			texcoords[index] = glm::vec3(static_cast<float>(i) / (static_cast<float>(longitude_split_count) - 1.0f), static_cast<float>(j) / (static_cast<float>(latitude_split_count) - 1.0f),
				0.0f);
	...
	// create index array
	auto index_sets = std::vector<glm::uvec3>(2u * (latitude_split_count - 1u) * (longitude_split_count - 1u));

	// generate indices iteratively
	index = 0u;
	for (unsigned int i = 0u; i < longitude_split_count - 1u; ++i)
	{
		for (unsigned int j = 0u; j < latitude_split_count - 1u; ++j)
		{
		

			index_sets[index] = glm::uvec3(latitude_split_count * (i + 0u) + (j + 0u),
				latitude_split_count * (i + 1u) + (j + 0u),
				latitude_split_count * (i + 1u) + (j + 1u));
			++index;

			index_sets[index] = glm::uvec3(latitude_split_count * (i + 0u) + (j + 0u),
				latitude_split_count * (i + 1u) + (j + 1u),
				latitude_split_count * (i + 0u) + (j + 1u));
			++index;
		}
	}
